 ...
 * @property {string} componentStatus - 'prompting' | 'reviewing' | 'incomplete' | 'completed' | 'failed' | 'disabled' | 'sandbox' | 'unknown'
...
export function modalityReducer(state, action) {
  switch (action.type) {
    ...
    case 'STATUS_CHANGED':
      const disabled =
        action.payload !== 'incomplete' && action.payload !== 'sandbox';
      return {
        ...state,
