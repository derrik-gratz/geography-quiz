...
export function QuizPrompt({}) {
  ...
  const currentPrompt = useMemo(() => {
    ...
    const promptValue = derivePromptValue(
      currentPromptData,
  ...
  const displayPrompt = (prompt) => {
    switch (prompt.type) {
      case 'location':
        return (
          <span className="prompt-location">
            {formatLatitude(prompt.value.lat)},{' '}
            {formatLongitude(prompt.value.long)}
          </span>
        );
      case 'name':
      ...
      case 'flag':
        return (
          <span
            className={`prompt-flag fi fi-${prompt.value.toLowerCase()}`}
          ></span>
        );
    }
  };
    ...
    if (state.config.gameMode === 'sandbox') {
      ...
      return (
        <span className="prompt-content__generic-text">
          ...
          <br />
          Next challenge in {timeUntilNextDay.hours}h:{timeUntilNextDay.minutes}
          m.
        </span>
      );
  ...
  const promptButton = useMemo(() => {
    ...
    if (state.quiz.status === 'not_started') {
      ...
      return (
        <button className="quiz-config__reset-quiz-button" onClick={resetQuiz}>
          Reset quiz
        </button>
      );
    }
    ...
    <CollapsibleContainer
      ...
      content={
        <div className="quiz-prompt">
          <div className="quiz-prompt__content">{promptContent}</div>
          <div className="quiz-prompt__button">{promptButton}</div>
        </div>
      }
    />
  );
