...
export function SelectedCountryDisplay({ selectedCountry, countryStats, displayMode }) {
    const dailyChallengeDataAvailable = selectedCountry && countryStats[selectedCountry] !== undefined 
    ? (() => {
    ...
    const countryData = allCountryData.find(country => country.code === selectedCountry);
    const countryFlag = countryData ? 
    <span className={`selected-country-display__flag fi fi-${countryData.flagCode.toLowerCase()}`}/> : '';
