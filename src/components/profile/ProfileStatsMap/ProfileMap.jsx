...
export function ProfileMap({ countryStats }) {
  ...
  const getCountryStyle = (countryCode) => {
    ...
    if (displayMode === 'dailyChallengeAccuracy') {
      ...
      displayMode === 'learningRate' &&
      countryStats[countryCode]?.learningRate &&
      countryStats[countryCode].lastChecked
    ) {
    ...
    return {
      default: {
        fill:
          countryCode === selectedCountry || countryCode === hoveredCountry
            ? 'var(--fill)'
            : fillColor,
        stroke: 'var(--stroke)',
